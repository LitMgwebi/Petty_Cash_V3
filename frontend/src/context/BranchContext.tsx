...
export const BranchProvider: FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const getBranches = async (): Promise<ServerResponse<Branch[]>> => {
        try {
            const res =
                await axios.get<ServerResponse<Branch[]>>("Branches/index");
    ...
    const getBranchById = async (
        id: number
    ): Promise<ServerResponse<Branch>> => {
        try {
            const res = await axios.get<ServerResponse<Branch>>(
                `Branches/details/${id}`
            );
