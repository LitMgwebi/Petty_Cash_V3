...
{
    public class GetAllIssuedState(IAuth auth) : IIndexState
    {
        ...
        public async Task<ServerResponse<List<Requisition>>> GetRequisitions(BackendContext db)
        {
            if (_auth.ValidateUserRole("Accounts_Payable"))
            {
                List<Requisition> requisitions = await db.Requisitions
                   ...
                   .Where(a => a.StateId == Status.Issued)
                   .AsNoTracking()
                   .ToListAsync();
                ...
                return requisitions == null ?
                    ...
                    {
                        Success = false,
                        Message = "System could not retrieve all of the issued requisitions. Please contact ICT."
                    } :
                    new ServerResponse<List<Requisition>>
                    {
                        Success = true,
                        Data = requisitions,
                        Message = "All of the issued requisitions have been retrieved."
                    };
            }
            else
            {
