...
export const DepartmentProvider: FC<{ children: ReactNode }> = ({
    children,
}) => {
    ...
    const getDepartments = async (): Promise<ServerResponse<Department[]>> => {
        try {
            const res =
                await axios.get<ServerResponse<Department[]>>(
                    "Departments/index"
                );
    ...
    const getDepartmentById = async (
        id: number
    ): Promise<ServerResponse<Department>> => {
        try {
            const res = await axios.get<ServerResponse<Department>>(
                `Departments/${id}`
            );
